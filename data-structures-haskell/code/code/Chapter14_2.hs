...
equalPair :: (a -> a -> Bool) -> Pairs a -> Bool
equalPair eq (Pr x y) = (eq x y)
...
-- The Maybe type, is defined in the Prelude.lhs,
...
transform (x:xs) (y:ys)
  | eqChar x y        = Copy : transform xs ys
