...
allEven xs = (eqList (==) xs [x | x<-xs, isEven x])
allOdd xs  = (eqList (==) [] [x | x<-xs, isEven x])
...
books dBase findPerson
  = [ book | (person,book) <- dBase , person `eqString` findPerson ]
...
returnLoan dBase pers bk
  = [ pair | pair <- dBase , not (eqTuple2 eqString eqString pair (pers,bk)) ]
